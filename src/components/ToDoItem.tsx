...
export default function ToDoItem({ task }: ToDoItemProps) {
  ...
  return (
    <li
      className={`p-3 rounded-md shadow-sm transition-all border border-gray-200 hover:shadow-md hover:border-gray-400 cursor-pointer ${task.status === TaskStatus.done ? 'bg-gray-100' : 'bg-white'}`}
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h3
            className={`text-base font-medium ${task.status === TaskStatus.done ? 'line-through text-gray-500' : ''}`}
          >
            {task.title}
          </h3>
          {task.isNew && task.status !== TaskStatus.done && (
      ...
      <blockquote
        className={`p-3 my-3 border-l-4 border-gray-200 bg-gray-50 ${task.status === TaskStatus.done ? 'line-through opacity-50' : ''}`}
      >
      ...
      <div className="flex justify-between mt-3">
        ...
        <div className="flex gap-3 items-center">
          <p className="text-xs text-gray-500">
            ...
            {String(task.dueDate.hour).padStart(2, '0')}:
            {String(task.dueDate.minute).padStart(2, '0')}
          </p>
          <p
            className={`text-xs font-medium py-1 px-2 rounded-full flex items-center border ${
              task.status === TaskStatus.done
